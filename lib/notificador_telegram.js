...
function gerarMensagem(dadosProcessados, dataVoo, arquivoTemplate = 'template_mensagem.txt') {
  ...
  // TODO: o pico pode ser também por quantidade de voos
  // Encontrar hora de pico
  let melhorHora = '00:00';
  let passageirosPico = 0;
  Object.entries(passageirosPorhora).forEach(([hora, dados]) => {
    if (dados.passageiros > passageirosPico) {
      passageirosPico = dados.passageiros;
      melhorHora = hora;
    }
  });
  ...
  Object.entries(passageirosPorhora).forEach(([hora, dados]) => {
    const { passageiros, voos } = dados;
    if (passageiros === 0 && voos === 0) return;

    const destaque = hora === melhorHora ? ' 🔥' : '';
    horariosComMovimento += `🕒 ${hora} | ✈️ ${String(voos).padStart(2, ' ')} | ${String(passageiros).padStart(4, ' ')} 👥${destaque}\n`;
  });
